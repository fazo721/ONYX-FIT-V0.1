...
function offerRecovery(){removeBanner();const a=active();if(!a?.workout||(!a.elapsed&&!a.sets?.length))return;const box=document.createElement('div');box.id='workoutRecoveryBanner';box.innerHTML=`<div style="position:fixed;left:12px;right:12px;bottom:88px;z-index:9998;background:#111;border:1px solid #3a3a3a;border-radius:16px;padding:14px;box-shadow:0 12px 35px #000"><b>⚡ Séance non terminée retrouvée</b><p style="margin:7px 0;color:#aaa;font-size:.86rem">${a.workout} · ${a.sets?.length||0} séries · ${Math.floor((a.elapsed||0)/60)} min. Tes données ne sont pas perdues.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn primary" id="recoverKeep">GARDER DANS L’HISTORIQUE</button><button class="btn" id="recoverDiscard">IGNORER</button></div></div>`;document.body.appendChild(box);const close=()=>{clear();removeBanner()};$('#recoverKeep')?.addEventListener('click',()=>{archiveRecovered(a);removeBanner()});$('#recoverDiscard')?.addEventListener('click',close)}
document.addEventListener('click',e=>{if(e.target.closest?.('#confirmStopWorkout,.confirm-stop-workout,#stopWorkoutConfirm'))setTimeout(()=>{clear();removeBanner()},50)},true);
const obs=new MutationObserver(()=>{if($('.workout-finished')){clear();removeBanner()}if(!active())removeBanner()});obs.observe(document.documentElement,{childList:true,subtree:true});
...
window.OnyxWorkoutAutosave={snapshot:snap,clear:()=>{clear();removeBanner()},active};
